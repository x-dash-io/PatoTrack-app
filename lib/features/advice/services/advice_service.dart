  ...
  List<AdviceInsight> generate({
    ...
    if (grossMargin < 0.30 && monthlyIncome > 0) {
      raw.add(_Raw(
        ...
        body: 'Your gross margin is ${(grossMargin * 100).toStringAsFixed(1)}%. '
        ...
        impact: 8, probability: 9, urgency: 6, complexity: 4,
      ));
    }
    if (operatingMargin > 0.20 && monthlyIncome > 0) {
      raw.add(_Raw(
        title: 'Strong Operating Margin — Reinvest Now',
        body: 'Your operating margin is ${(operatingMargin * 100).toStringAsFixed(1)}%. '
            'This is a good time to reinvest in marketing, equipment, or inventory.',
        category: AdviceCategory.performance,
        impact: 7, probability: 8, urgency: 4, complexity: 2,
      ));
    }
    if (expenseGrowing && !incomeGrowing) {
      raw.add(_Raw(
        ...
        category: AdviceCategory.performance,
        impact: 9, probability: 7, urgency: 8, complexity: 3,
      ));
    }
    ...
    if (runway < 60 && monthlyExpense > 0) {
      raw.add(_Raw(
        ...
        impact: 10, probability: 9, urgency: 10, complexity: 2,
      ));
    }
    if (incomeConcentration > 0.65) {
      raw.add(_Raw(
        title: 'Revenue Concentration Risk',
        body: '${(incomeConcentration * 100).toInt()}% of income flows from one '
            'source. Diversify — add at least two new revenue streams in the next 90 days.',
        category: AdviceCategory.risk,
        impact: 8, probability: 8, urgency: 6, complexity: 3,
      ));
    }
    if (anomalyCount > 2) {
      raw.add(_Raw(
        ...
        category: AdviceCategory.risk,
        impact: 7, probability: 8, urgency: 8, complexity: 2,
      ));
    }
    ...
    if (incomeGrowing && grossMargin >= 0.30) {
      raw.add(_Raw(
        ...
        impact: 7, probability: 7, urgency: 5, complexity: 3,
      ));
    }
    if (burnRate > 0 && monthlyIncome > 0 && monthlyIncome / burnRate / 30 > 2) {
      raw.add(_Raw(
        ...
        category: AdviceCategory.opportunity,
        impact: 6, probability: 8, urgency: 3, complexity: 2,
      ));
    }
